...
select function_returns(
    'drv_min',
    array[
        'real','real','timestamp without time zone','integer'
        ],
    'void'
);
...
prepare fail_non_driver as 
    select drv_min(16.911553,121.060724,now()::timestamp,10); 
select throws_ok(
    'fail_non_driver',
    'code:1'
);
...
select drv_min(16,121,now()::timestamp,5); 
...
prepare invalid_lat as select drv_min(0.1,121,now()::timestamp,5);  
select throws_ok('invalid_lat','code:1');


prepare invalid_lat2 as select drv_min(22.1,121,now()::timestamp,5);  
select throws_ok('invalid_lat2','code:1');


prepare invalid_long as select drv_min(1,115.9,now()::timestamp,5);  
select throws_ok('invalid_long','code:1');


prepare invalid_long2 as select drv_min(21,128.1,now()::timestamp,5);  

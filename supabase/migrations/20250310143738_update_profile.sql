...
drop function public.update_profile(text[],text);
...
is_male_secret:= vault.create_secret(new.raw_user_meta_data ->> 'is_male')::uuid;
...
Values (
  ...
  new.raw_user_meta_data ->> 'username',is_male_secret
);
...
CREATE OR REPLACE FUNCTION public.update_profile(data jsonb)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
  ...
  select ARRAY(SELECT jsonb_object_keys(data)) into arr_key;
  if (arr_key <@ array['nme','ads','bday','phn','str','gdr','u_name'])=false then
  ...
  SELECT ARRAY(
      ...
      FROM jsonb_array_elements_text(jsonb_extract_path(data,'nme')) AS x
  ) INTO nme_trim;
  SELECT ARRAY(
      SELECT trim(initcap(x))
      FROM jsonb_array_elements_text(jsonb_extract_path(data,'ads')) AS x
  ) INTO ads_trim;
  SELECT ARRAY(
      SELECT trim(initcap(x)) 
      FROM unnest(string_to_array(data->>'str',',')) AS x
  ) INTO str_trim;
  SELECT TRIM(jsonb_extract_path(data,'bday')::text) INTO bday_trim;
  SELECT TRIM(data->>'phn') INTO phn_trim;
  SELECT TRIM(data->>'gdr') INTO gdr_trim;
  SELECT TRIM(data->>'u_name') INTO usname;
  ...
  if error_msg !='' then
    ...
    if bday_trim is not null and arr_err[4] is null then
      if secrets[4] is null and bday_trim is not null  then
        uuids[4]:=vault.create_secret(bday_trim[1])::uuid;
        update public.profile
        set bday = uuids[4]
        where id= (select auth.uid());
        result =true;
...
begin
  ...
  select nme_d::text,street::text,complete_brgy::text,phn_d::text,bdy_d::text,gdr_d::text,unme::text into result;
  return result;
  end$function$
;
revoke execute on function update_profile(jsonb) from public,authenticated,anon;
grant execute on function update_profile(jsonb) to authenticated,postgres;
alter table public.profile
drop column is_male;
alter table public.profile
add column is_male uuid;
alter table "public"."profile" add column "updated_at" timestamp with time zone;
ALTER TABLE public.profile
ADD CONSTRAINT profile_is_male_fkey
FOREIGN KEY (is_male) REFERENCES vault.secrets(id) ON UPDATE CASCADE;
